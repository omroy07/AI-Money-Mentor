...
function updatePortfolioUI(holdings, summary) {
  // Update summary
  document.getElementById('portTotalInvested').innerHTML =
    `₹${fmtNum(summary.total_invested)}`;
  document.getElementById('portCurrentValue').innerHTML =
    `₹${fmtNum(summary.total_current)}`;
  document.getElementById('portTotalPnL').innerHTML =
    `<span class="positive">+₹${fmtNum(summary.total_pnl)}</span>`;
  document.getElementById('portReturns').innerHTML =
    `<span class="positive">+${summary.total_pnl_percent}%</span>`;
  ...
  if (canvas) {
    ...
    portfolioChart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          legend: { position: 'bottom', labels: { color: '#eef0f5' } },
        },
      },
    });
  }
  ...
  const sorted = [...holdings].sort(
    (a, b) => parseFloat(b.pnlPercent) - parseFloat(a.pnlPercent),
  );
  document.getElementById('topPerformers').innerHTML = sorted
    .map(
      (p) => `
...
async function loadAlerts() {
  ...
  if (data.success && data.alerts.length) {
    alertsDiv.innerHTML = data.alerts
      .map(
        (a) =>
          `<div style="display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid var(--border);"><span><strong>${esc(a.symbol)}</strong> ${esc(a.condition)} ₹${a.target_price}</span><span class="${a.is_triggered ? 'positive' : ''}">${a.is_triggered ? '✓ Triggered' : '● Active'}</span></div>`,
      )
      .join('');
  } else {
    alertsDiv.innerHTML =
      '<div style="text-align:center;padding:20px;color:var(--muted);">No alerts set</div>';
  }
}
...
async function checkAlerts() {
  ...
  if (data.success && data.triggered.length)
    alert(
      `🔔 ${data.triggered.map((t) => `${t.symbol} reached ₹${t.current}`).join(', ')}`,
    );
...
function exportCSV() {
  fetch('/portfolio/list')
    ...
    .then((data) => {
      ...
      let csv =
        'Symbol,Quantity,Buy Price,Current Price,Invested,Current Value,P&L\n';
      data.holdings.forEach(
        (h) =>
          (csv += `${h.symbol},${h.quantity},${h.buy_price},${h.current_price},${h.invested_value},${h.current_value},${h.pnl}\n`),
      );
...
function clearChat() {
  ...
  appendMsg(
    'chatMessages',
    'bot',
    "Hello! I'm your AI financial advisor. How can I help?",
  );
}
...
async function calcSIP() {
  const res = await fetch('/sip', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      monthly: parseFloat(document.getElementById('sip_monthly').value),
      rate: parseFloat(document.getElementById('sip_rate').value),
      years: parseInt(document.getElementById('sip_years').value),
    }),
  });
  const data = await res.json();
  document.getElementById('sipResult').innerHTML =
    `<div class="positive" style="font-size:24px;font-weight:800;">₹${fmtNum(data.future_value)}</div>`;
}
...
async function checkStock() {
  const res = await fetch('/portfolio', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ stock: document.getElementById('stockSym').value }),
  });
  const data = await res.json();
  document.getElementById('stockResult').innerHTML =
    `<div><strong>${esc(data.symbol)}</strong> ₹${fmtNum(data.price)}</div><div>${esc(data.analysis)}</div>`;
}
...
async function calcTax() {
  const res = await fetch('/tax', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      income: parseFloat(document.getElementById('taxIncome').value),
    }),
  });
  const data = await res.json();
  document.getElementById('taxResult').innerHTML =
    `<div class="positive" style="font-size:24px;font-weight:800;">Tax: ₹${fmtNum(data.tax)}</div>`;
}
...
async function calcScore() {
  const res = await fetch('/money-score', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      income: parseFloat(document.getElementById('s_income').value),
      expenses: parseFloat(document.getElementById('s_expenses').value),
      savings: parseFloat(document.getElementById('s_savings').value),
      investments: parseFloat(document.getElementById('s_invest').value),
      debt: parseFloat(document.getElementById('s_debt').value),
      emergency: parseFloat(document.getElementById('s_emergency').value),
    }),
  });
  const data = await res.json();
  document.getElementById('scoreResult').innerHTML =
    `<div class="positive" style="font-size:24px;font-weight:800;">Score: ${data.score}</div><div>${esc(data.status)}</div>`;
}
...
async function detectAnomalies() {
  ...
  const data = await res.json();
  document.getElementById('anomaliesResult').innerHTML = data.anomalies?.length
    ? data.anomalies
        .map((a) => `<div class="performer-card">⚠️ ${esc(a.reason)}</div>`)
        .join('')
...
async function loadNetWorth() {
  ...
  document.getElementById('nwLiabilities').innerHTML =
    `₹${fmtNum(data.total_liabilities)}`;
  document.getElementById('nwTotal').innerHTML = `₹${fmtNum(data.net_worth)}`;
  document.getElementById('assetList').innerHTML = data.assets
    .map(
      (a) =>
        `<div style="padding:8px;border-bottom:1px solid var(--border);">${esc(a.name)}: ₹${fmtNum(a.amount)}</div>`,
    )
    .join('');
  document.getElementById('liabList').innerHTML = data.liabilities
    .map(
      (l) =>
        `<div style="padding:8px;border-bottom:1px solid var(--border);">${esc(l.name)}: ₹${fmtNum(l.amount)}</div>`,
    )
    .join('');
}
...
async function addNWItem(type) {
  ...
  const amount = parseFloat(
    document.getElementById(type === 'asset' ? 'assetAmt' : 'liabAmt').value,
  );
...
function addBudgetCategory() {
  ...
  if (!name || budgeted <= 0) {
    alert('Enter a category name and budgeted amount.');
    return;
  }

  const existing = budgetCategories.findIndex(
...
function updateBudgetStats() {
  ...
  document.getElementById('bRemaining').innerHTML =
    `<span class="${remaining < 0 ? 'negative' : 'positive'}">₹${fmtNum(remaining)}</span>`;
}
    ...
    budgetPieChart = new Chart(pieCanvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: { color: '#eef0f5', font: { size: 11 } },
          },
        },
      },
    });
  ...
  if (barCanvas) {
    ...
    budgetBarChart = new Chart(barCanvas.getContext('2d'), {
      ...
      options: {
        ...
        maintainAspectRatio: true,
        plugins: {
          legend: { labels: { color: '#eef0f5', font: { size: 11 } } },
        },
        scales: {
          x: {
            ticks: { color: '#5a6a82' },
...
async function analyzeBudget() {
  const income = parseFloat(document.getElementById('budgetIncome').value) || 0;
  if (!income) {
    alert('Enter your monthly income first.');
    return;
  }
  if (!budgetCategories.length) {
    alert('Add at least one budget category first.');
    return;
  }
  ...
  adviceDiv.innerHTML =
    '<div style="color:var(--muted);font-size:13px;">Asking AI advisor…</div>';
...
function resetBudget() {
  ...
  document.getElementById('budgetMonth').value = new Date()
    .toISOString()
    .slice(0, 7);
